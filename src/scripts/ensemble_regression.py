...
models = {
    ...
    "LightGBM": {
        ...
        "param_grid": {
            "learning_rate": [0.01, 0.1],
            "n_estimators": [100, 200],
            "num_leaves": [30, 42, 50, 100],
        },
    },
}


# Function to perform grid search for all models
def perform_grid_search_all_models(X_train, y_train, X_test):
    best_models = {}
    y_preds = {}

    # Convert data to CuPy arrays for GPU-accelerated models
    ...
    for model_name, model_info in models.items():
        ...
        # Fit the model (for XGBoost and models that support GPU, use CuPy arrays)
        ...
        # Predict on the test set (convert to CuPy array for GPU-supported models)
        if model_name in ["XGBoost"]:
            y_preds[model_name] = best_model.predict(X_test_cp)
        else:
...
for pred_horizon, target_col in targets.items():
    ...
    # )
    # best_models[pred_horizon], y_pred[pred_horizon] = perform_grid_search_all_models(
    #     cp.array(X_train[pred_horizon]),
    #     cp.array(y_train[pred_horizon][target_col]),
    #     cp.array(X_test[pred_horizon]),
    # )
    best_models[pred_horizon], y_pred[pred_horizon] = perform_grid_search_all_models(
        X_train[pred_horizon],
        y_train[pred_horizon][target_col],
        X_test[pred_horizon],
    )
    ...
    for model_name, model_info in models.items():
        ...
        y_pred_df = pd.DataFrame(
            y_pred[pred_horizon][model_name],
            index=X_test[pred_horizon].index,
            columns=[target_col],
        )
        print("y pred df:\n", y_pred_df)
        save_test_data(
            ...
            pred_horizon,
            "forecast",
            f"./forecasts_ensemble/forecast{pred_horizon}/LIVERPOOL_forecast1_{seq_length}_{pred_horizon}_{model_name}.csv",
        ...
        evaluation_results.to_csv(
            f"./forecasts_ensemble/stats/complete/forecast{pred_horizon}/LIVERPOOL_stats1_{seq_length}_{pred_horizon}_{model_name}.csv",
            index=False,
        )
