...
for pred_length in pred_lengths:
    ...
    for scenario_id, config in scenarios.items():
        ...
        scenario_models[str(pred_length)][scenario_id] = {
            ...
            "y_train": y_train,
            "y_test": y_test,
            "label_encoder": shared_label_encoder,  # Save the encoder to decode labels later
        }
...
for pred_length, scenarios in scenario_models.items():
    ...
    for scenario_id, data in scenarios.items():
        ...
        # Check if y_train and y_test are balanced across the classes
        ...
        y_pred = lstm_model.predict(X_test)  # Or use GRU or XGBoost model
        ...
        print(
            f"Scenario {scenario_id}, Prediction Length {pred_length}, Accuracy: {accuracy}"
        )
