...
for pred_length in pred_lengths:
    ...
    for scenario_id, config in scenarios.items():
        ...
        X = pd.read_csv(
            f"./fusion_dsi/partial/forecast{pred_length}/dsi_ensemble_{int(int(pred_length)/2)}_{pred_length}_ID{scenario_id}_v3.csv",
            index_col="Segment",
        ...
        y = pd.read_csv(
            f"./fusion_dsi/complete/forecast{pred_length}/dsi_ensemble_{pred_length}_{pred_length}_ID{scenario_id}_v3.csv",
            index_col="Segment",
        )
...
param_grid_rf = {
    ...
    "max_depth": [2, 3, 4, 5, 6, 7, 8, 9, 10, 12, 15],  # Maximum depth of the trees
    "min_samples_split": [
        2,
        3,
        4,
...
for pred_length, scenarios in scenario_models.items():
    ...
    for scenario_id, data in scenarios.items():
        ...
        # Meta-learner (you can use Logistic Regression or any other model)
        meta_model = LogisticRegression()

        # Define stacking classifier using the base models and meta-model
        stacked_model = StackingClassifier(
            estimators=[
                ("xgb", best_model_xgb),  # Best XGBoost model
                ("rf", best_rf_classifier),  # Best Random Forest model
            ],
            final_estimator=meta_model,  # Meta-learner model
        ...
        print(
            f"Classification Report for Stacked Model of scenario {scenario_id}-{pred_length}h:\n"
        )
